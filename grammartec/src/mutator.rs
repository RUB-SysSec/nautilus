...
use context::Context;
use newtypes::NodeID;
use rule::{NormalOrCustomRule, Rule};
use tree::{Tree, TreeLike, TreeMutation};

use afl_mutator::MutationState;
...
impl Mutator {
        ...
        F: FnMut(&TreeMutation, &HashSet<usize>, &Context) -> Result<bool, SubprocessError>,
    {
        ...
        while i < tree.size() {
            ...
            if let Some(parent) = Mutator::find_parent_with_nt(tree, n, ctx) {
                if let Some(t) = Mutator::test_and_convert(tree, parent, tree, n, ctx, bits, tester)?
                {
                    mem::replace(tree, t);
                    i = parent.into();
                }
        ...
        F: FnMut(&TreeMutation, &Context) -> Result<(), SubprocessError>,
    {
        for i in start_index..end_index {
            ...
            let n = NodeID::from(i);
            match tree.get_rule_id(n) {
                Some(old_rule_id) => {
                    let rule_ids = ctx.get_rules_for_nt(ctx.get_nt(old_rule_id)).to_vec(); //TODO: Maybe find a better solution
                    for new_rule_id in rule_ids {
                        if old_rule_id != new_rule_id {
                            ...
                            let repl =
                                tree.mutate_replace_from_tree(n, &self.scratchpad, NodeID::from(0));
                            tester(&repl, ctx)?;
                        }
                    }
                }
        ...
        F: FnMut(&TreeMutation, &Context) -> Result<(), SubprocessError>,
    {
        for i in start_index..end_index {
            ...
            let n = NodeID::from(i);
            let mut data: Vec<u8> = tree.unparse_node_to_vec(n, ctx);
            if data.len() < 4 {
                continue;
            } //Afl_mutations need inputs of min len 4 bytes
            let start_data = data.clone(); //save original state
            let mut mutator = MutationState::new_bitflip(vec![1; data.len()]);
            let mut range = mutator.deterministic(&mut data);
            while { range.is_some() } {
                let new_custom_rule =
                    Rule::from_custom_term(tree.get_rule(n, ctx).nonterm(), data.clone());
                let rule_vec = vec![NormalOrCustomRule::CustomRule(new_custom_rule)];
                let repl_tree = Tree::from_rule_vec(rule_vec, ctx);
                let repl = tree.mutate_replace_from_tree(n, &repl_tree, NodeID::from(0));
                tester(&repl, ctx)?;

                //Reverse changes
                for x in range.expect("RAND_3102313384") {
                    data[x] = start_data[x];
                }
                range = mutator.deterministic(&mut data);
            }
        }
        return Ok(false);
    }
        ...
        F: FnMut(&TreeMutation, &Context) -> Result<(), SubprocessError>,
    {
        ...
        if let Some(recursion) = rand::thread_rng().choose(&recursions) {
            ...
            for i in 0..num_of_recursions * recursion_len_pre {
                rules_new.push(
                    tree.get_normal_rule_or_custom_rule(recursion.0 + (i % recursion_len_pre))
                        .clone(),
                );
            ...
            for i in 0..postfix {
                rules_new.push(tree.get_normal_rule_or_custom_rule(recursion.1 + i).clone());
            ...
            for i in 0..num_of_recursions * recursion_len_post {
                rules_new.push(
                    tree.get_normal_rule_or_custom_rule(
                        recursion.1 + postfix + (i % recursion_len_post),
                    ).clone(),
                );
...
mod tests {
    ...
    fn check_mut_random_recursion() {
        let r1 = NormalOrCustomRule::NormalRule(RuleID::from(0));
        let r2 = NormalOrCustomRule::NormalRule(RuleID::from(1));
        let r3 = NormalOrCustomRule::NormalRule(RuleID::from(2));
        let r4 = NormalOrCustomRule::NormalRule(RuleID::from(3));
        let r5 = NormalOrCustomRule::NormalRule(RuleID::from(4));

        let n0 = NodeID::from(0);
        let n1 = NodeID::from(1);
        let n3 = NodeID::from(3);
        let n4 = NodeID::from(4);
        ...
        ctx.add_rule("N1", "r1{N2}{N3}{N4}");
        ctx.add_rule("N2", "r2");
        ctx.add_rule("N3", "r3{N1}");
        ctx.add_rule("N1", "r4");
        ctx.add_rule("N4", "r5");
        ...
        let mut tester = |tree_mut: &TreeMutation, _ctx: &Context| {
            ...
            assert!(tree_mut.prefix == [r1.clone(), r1.clone(), r2.clone(), r3.clone()]);
            assert!(tree_mut.postfix == [r5.clone()]);

            assert!(tree_mut.repl[0..3] == [r1.clone(), r2.clone(), r3.clone()]);
            assert_eq!(tree_mut.repl.last(), Some(&r5));
            return Ok(());
        };
        println!("tree: ");
        let recursions = tree.has_recursions(&ctx).expect("RAND_3407743327");
        mutator
    ...
    fn check_minimize_tree() {
        let mut ctx = Context::new();
        let r1 = ctx.add_rule("S", "s1 {A}");
        let _ = ctx.add_rule("S", "s2");
        let _ = ctx.add_rule("S", "a1");
        let r2 = ctx.add_rule("A", "a1 {B}");
        let _ = ctx.add_rule("A", "a1");
        let _ = ctx.add_rule("A", "a2");
        let r3 = ctx.add_rule("B", "b1");
        let _ = ctx.add_rule("B", "b2");
        let _ = ctx.add_rule("B", "b3{B}");
    ...
    fn check_det_rules_values() {
        let mut ctx = Context::new();
        let r1 = ctx.add_rule("S", "s1 {A}");
        let _ = ctx.add_rule("S", "s2 {A}");
        let r2 = ctx.add_rule("A", "a1 {B}");
        let _ = ctx.add_rule("A", "a2 {B}");
        let r3 = ctx.add_rule("B", "b1");
        let _ = ctx.add_rule("B", "b2");
        ctx.initialize(10, false);
        for _ in 0..100 {
    ...
    #[test]
    fn check_det_afl_rules_values() {
        let mut ctx = Context::new();
        let r1 = ctx.add_rule("S", "1{A}");
        let r2 = ctx.add_rule("A", "1{B}");
        let r3 = ctx.add_rule("B", "11"); //'1' is 00110001 in ascii
        ...
        let mut unparses = HashSet::new();
        {
            let mut tester = |tree_mut: &TreeMutation, ctx: &Context| {
                print!("[");
                for x in tree_mut.unparse_to_vec(&ctx) {
                    print!("{:08b}", x);
                }
                println!("]");
                unparses.insert(tree_mut.unparse_to_vec(&ctx));
            ...
            mutator
                .mut_rules_afl(&tree, &ctx, 0, tree.size(), &mut tester)
                .expect("RAND_1378672390");
        }
        //Check for some bitflips
        assert!(unparses.contains(&vec![0b10110001, 0b00110001, 0b00110001, 0b00110001]));
        assert!(unparses.contains(&vec![0b01110001, 0b00110001, 0b00110001, 0b00110001]));
        assert!(unparses.contains(&vec![0b00010001, 0b00110001, 0b00110001, 0b00110001]));
        assert!(unparses.contains(&vec![0b00111001, 0b00110001, 0b00110001, 0b00110001]));
        assert!(unparses.contains(&vec![0b00110101, 0b00110001, 0b00110001, 0b00110001]));

        assert!(!unparses.contains(&vec![0b00110001, 0b00110001, 0b00110001, 0b00110001])); //should not contain the original input "1111"
    }
